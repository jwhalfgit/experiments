...
plotTrajDiff <- function(DF, FILTER, SITE1, SITE2){
  ...
  # Make the plot!
  ggplot() + 
    geom_sf(data = grid_sf, aes(fill = dcount, color = NA),color = NA,alpha = 0.8)+
    # #scale_fill_gradientn(colors = rainbow(7), name = "Difference") +
    scale_fill_distiller(palette = "RdYlBu", limits = c(-1000,1000),name = "Difference", trans = pseudolog10_trans,
                          breaks=c(-1000,-100,-10,0,10,100,1000),na.value = "transparent") +
    theme_bw()+
...
calcDiff <- function(SITE1,SITE2 = bss, 
                     DATE1 = "01/01/2023", DATE2 = "31/03/2023",
...
seasDates <- data.frame(START = c("01/01/2023","01/04/2023","01/07/2023", "01/10/2023"),
                        STOP = c("31/03/2023","30/06/2023","30/09/2023", "31/12/2023"))
DATE1 <- "01/04/2023"
DATE2 <- "30/06/2023"
